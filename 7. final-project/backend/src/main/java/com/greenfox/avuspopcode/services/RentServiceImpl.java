...
public class RentServiceImpl implements RentService {
  ...
  @Override
  public Set<Rent> getRentsByUserIdAndStatus(Long userId, List<String> statusList) {
    List<RentStatus> statusEnums = new ArrayList<>();
    for (String status : statusList) {
      statusEnums.add(RentStatus.valueOf(status));
    }
    return new HashSet<>(this.rentRepository.findByUserIdAndStatus(userId, statusEnums));
  }
  ...
  public void updateRentsStatus(RentPutDTO rentPutDTO) {
    Set<Rent> updatedRents = new HashSet<>();
  ...
  @Override
  public Set<Rent> findByUserId(Long userId) {
    return this.rentRepository.findByUserId(userId);
  }
}